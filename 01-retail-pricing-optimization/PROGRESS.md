...
**Real feature gap found post-launch (2026-08-06), while explaining "Available inventory override" to a user:** it only existed on Scenario Explorer -- Decision Replay and Adaptive Learning had no way to test a tighter/looser starting stock position at all. Added `inventory_override` to both `run_closed_loop_replay` (`src/replay_engine.py`) and `run_adaptive_simulation` (`src/adaptive_simulation.py`), and a matching UI control on both pages. Necessarily a *different* semantic than Scenario Explorer's version: these two pages are closed loops where only **Day 1's** stock comes from a fixed number (real history, or the override) — every later day is already determined by the model's own prior decisions plus replenishment, so the override can only ever seed the starting point, never override an individual later day. In Adaptive Learning, the override applies uniformly to all 3 variants (Static/Thompson/Oracle) to keep the three-way comparison fair. Also simplified the "Available inventory override" tooltip wording on all 3 pages (previously assumed familiarity with the concept) and clarified Risk Aversion's help text everywhere it appears to state its direction in plain terms (higher = lower stockout chance) after a user asked which direction it went. 4 new regression tests (`test_replay_engine.py`, `test_adaptive_learning.py`, 2x `test_app_pages.py`). 99/99 tests passing.

**Real deployment gotcha found immediately after this push (2026-08-06):** the live app crashed with `TypeError: run_closed_loop_replay() got an unexpected keyword argument 'inventory_override'` right after deploying the change above. Verified directly (diffed local vs. `origin/main`) that the pushed code was 100% correct on GitHub -- the live server was simply still running a stale, already-imported copy of `src/replay_engine.py` in memory. Streamlit Cloud's automatic "Updated app!" reload re-runs the page scripts but doesn't reliably force Python to re-import `src/`-level modules that were already loaded earlier in the process's lifetime; only a manual **Reboot** (Manage app → Reboot) guarantees a fresh import of everything. No code was broken or changed to fix this -- documented in `DEPLOYMENT.md` so a `src/`-only change doesn't cost another investigation next time.

**Real methodology gap found post-launch (2026-08-06), while explaining to a user why Adaptive Learning has no promotion/event/day-of-week controls:** the online belief was a plain 1-parameter regression (`dlog_units ~ beta*dlog_price`), so any promotion or event that moved alongside a price change got misread as price sensitivity — the same omitted-variable bias an uncontrolled difference-in-differences estimate has. Before adding those as free parameters (which would reintroduce the exact 2-parameter instability documented below), first checked how reliably Phase 3's *already-fitted* promo/event/dow coefficients were estimated: pulled live diagnostics from the cached posterior (`az.summary` on `gamma_promo`/`gamma`) — every coefficient's 95% credible interval excludes zero, `r_hat` is 1.00-1.01 everywhere, and effective sample sizes are all well above the 400 minimum (lowest: day-of-week's ~700-760; highest: `gamma_promo`'s ~5,000-6,700). Concluded these were reliable enough to reuse, and residualized instead of re-estimated: `scripts/build_app_data.py` now exports posterior-mean promo/event/dow effect sizes to `data/app/control_effects.csv` (channel/region/marketing-spend coefficients also exist in the posterior but aren't exported — the app's daily data doesn't carry those at its aggregated grain), and `bayesian_learning.control_log_effect` subtracts the effect delta between each day and its anchor from `dlog_units` before it reaches `update_posterior`. Still exactly one free parameter (elasticity); nothing new is being estimated online. Verified the mechanism directly with a synthetic confound test (recovers true beta far better than the raw update when price and the confound are correlated), and verified it's live on real data: in a 20-day window spanning Black Friday/Cyber Monday, the elasticity posterior's 90% credible interval narrowed 35-50% across one SKU per product type versus the same run with residualization disabled, with mixed but mostly favorable effects on regret. 5 new tests in `tests/test_adaptive_learning.py`. 104/104 tests passing.
